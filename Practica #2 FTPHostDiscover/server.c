/*
*
*       En la terminal: ./server <Broadcast Port> <TCP Port> <Name> <Dir Route>
*
...
int main(int args, char *argv[]){
        ...
        if(child==0){
                ...
                while(1){
                        ...
                        fprintf(stderr, "> Se ha conectado el cliente [%s:%i]", senderIp, senderPort);
 
                        if((strstr("Saludos del cliente", buffer))==NULL){
 
                                sprintf(archivos, "%d", cntDir(argv[4]));
                                nombre = calloc(strlen(archivos)+strlen(argv[4])+strlen(argv[3])+29,1);
                                sprintf(nombre, "> Servidor: %s\r\n> Puerto: %s\n> Archivos: %d\r\n\r\n", argv[3], argv[2], cntDir(argv[4]));
        ...
        if(child>0){
                ...
                char* wantItem;
                char fileSize [10];
                char commandRequest[15];  //Variable para guardar el comando a utilizar
                ...
                while(1){
                        ...
                        do{
                                ...
                                status = read(client, commandRequest, sizeof(commandRequest));
                                bzero(buffer, sizeof(buffer));
 
                                if((strcmp("FILELIST", commandRequest))==0){
                                        status = read(client, buffer, sizeof(buffer));  //Recibimos el GET
                                        printf("Enviando lista");
                                        status = write(client, "OK FILELIST\r\n", strlen("OK FILELIST\r\n"));  //Enviamos el OK
                                        TCPsendDir(client, argv[4]);  //Enviamos la lista en base a lsDir-2.h
                                }
 
                                if((strcmp("GETFILE", commandRequest))==0){
                                        printf("> El cliente solicitó la lista de archivos");
                                        status = read(client, buffer, sizeof(buffer));  //Recibimos el GET
                                        wantItem = strtok(buffer, " ");
                                        for(i = 0; i < 2; i++)
                                                wantItem = strtok(NULL, " ");  //Separamos la cadena para encontrar el nombre del archivo
                                        printf("\n> El cliente quiere el archivo: '%s'\n",wantItem);
                        ...
                        //iniciamos la transferencia del archivo
                                ...
                                if((strcmp("PING", commandRequest))==0){
                                        status = write(client, "Pong", strlen("Pong"));
                                        printf("> El cliente nos mandó: PING\n> Le contestaré Pong");
                                }
 
                                if((strcmp("GETFILESIZE", commandRequest))==0){
                                        status = read(client, buffer, sizeof(buffer));  //Recibimos el GET
                                        wantItem = strtok(buffer, " ");
                                        for(i = 0; i < 2; i++)
                        ...
                        //Indicamos a la estructura cual sera el archivo para que obtenga sus propiedades
                                ...
                                if((strstr("EXIT", commandRequest))!=NULL){
