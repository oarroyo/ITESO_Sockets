...
//Se crea el socket para UDP, en caso de que no se pueda crear nos imprime un mensaje de error.
	...
	if(file == -1) {
		...
		sendto(udpSocket, "No encontrado\n\r", strlen("No encontrado"),0,(struct sockaddr*)&udpClient,sizeof(udpClient));
	return -1;
	}
  
	fstat(file,&filestat);

	sprintf(buffer,"Encontrado [%i] bytes LISTO para transferencia",filestat.st_size);
